 ...
 * Real-time Stock Quote Monitor & Low Price Smart Alert Engine (RWD Version)
 */
...
(function () {
  ...
  let config = {
    ...
    cooldownMs: 180000, // 3 mins default
    refreshInterval: 1000, // Default to 1 second for ultra-fast updates
    soundMuted: false,
    theme: 'light'
  };
  ...
  function init() {
    ...
    // Initial dummy data fill for chart
    seedPriceHistory();
    ...
    // Responsive Chart Resize Handling (Debounced)
  ...
  // --- Real-Time Data Fetching (Optimized for High Frequency) ---
  async function fetchRealtimeQuote() {
    ...
    try {
      ...
      if (data) {
        stockData = data;
        el.marketStatusText.textContent = '連線中';
        el.marketDot.className = 'pulse-dot';
      } else {
        generateSimulationTick();
        el.marketStatusText.textContent = '模擬中';
      ...
      el.marketStatusText.textContent = '重試中';
    } finally {
      isFetching = false;
    }
  }
  ...
  function seedPriceHistory() {
    ...
    for (let i = 30; i >= 0; i--) {
      ...
      const jitter = (Math.sin(i / 3) * 0.4) + ((Math.random() - 0.5) * 0.2);
      priceHistory.push({
        time: timeStr,
  ...
  function setupEventListeners() {
    ...
    el.btnTestAlert.addEventListener('click', () => {
      initAudioContext();
      fireAlert('【模擬測試】當前成交價 (74.0 元) ≤ 設定門檻 (74.0 元)', config.targetPrice || stockData.price);
    });

    el.chipMinus05.addEventListener('click', () => {
      const p = Math.max(1, stockData.price - 0.5);
      el.targetPriceInput.value = p.toFixed(1);
      el.btnSaveAlert.click();
    });

    el.chipMinus1.addEventListener('click', () => {
      const p = Math.max(1, stockData.price - 1.0);
      el.targetPriceInput.value = p.toFixed(1);
      el.btnSaveAlert.click();
    });

    el.chipMinus2Percent.addEventListener('click', () => {
      const p = Math.max(1, +(stockData.price * 0.98).toFixed(1));
      el.targetPriceInput.value = p.toFixed(1);
      el.btnSaveAlert.click();
    });
