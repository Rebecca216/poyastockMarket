...
Python Real-Time Stock Quote Monitor & Low Price Alert
"""
...
def main():
    ...
    print("=" * 65)
    print("時間      | 成交價   | 漲跌      | 買價(Bid) | 賣價(Ask) | 成交量(張)")
    print("-" * 65)
    ...
    while True:
        try:
            ...
            if quote:
                ...
                print(f"{t}  | {color}{p:6.1f}{reset}   | {color}{chg_str:12}{reset}| {bid:6.1f}    | {ask:6.1f}    | {vol:6d}")
                ...
                if is_alert:
                    ...
                    if now - last_alert_time >= cfg['cooldown_seconds']:
                        last_alert_time = now
                        print("\n" + "!" * 65)
                        print(f" 🚨【低價通知觸發】 寶雅 (5904) 股價達到低價目標！")
                        print(f" 📌 原因: {reason}")
